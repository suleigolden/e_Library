...
$sql312 = "SELECT * FROM borrowlibbookin ";
		...
		while($rocomname22 = mysql_fetch_array($query312)){
			...
			$status_book = $rocomname22['borrowedsatusbook'];
		...
		while($roctitle= mysql_fetch_array($querytitle)){
			...
			/**
		$sqlstatus = "SELECT * FROM borrowdetails WHERE book_id = '$get_book_borrowed' AND borrow_id = '$Member_borrowed'";
		$querystatus = mysql_query($sqlstatus);
			$status_book = mysql_num_rows($querystatus);**/
			if($status_book == "Pending"){
			$get_book_status = '<label style="color:#F00;">Pending</label>';
			}else{
			$get_book_status = '<label style="color:green;">Return</label>';
			}
...
?>
      ...
      <div id="page-wrapper" style="width:100%; margin-left:-9%;">
         ...
         <div class="row">
          <div class="col-lg-12">
            <h1><small>Borrowed Books</small></h1>
